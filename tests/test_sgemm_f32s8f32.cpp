...
// Adapted from test_sgemm_f32u4f32.cpp
...
// Test Fixture for SgemmF32S8F32 Quantize tests
class SgemmF32S8F32QuantizeTest : public ::testing::Test {
protected:
...
struct SgemmF32S8F32QuantizeParams {
...
class SgemmF32S8F32QuantizeParamTest : public SgemmF32S8F32QuantizeTest,
                                       public ::testing::WithParamInterface<SgemmF32S8F32QuantizeParams> {};

TEST_P(SgemmF32S8F32QuantizeParamTest, XdnnSgemmF32S8F32Quantize) {
    SgemmF32S8F32QuantizeParams p = GetParam();
    ...
    xdnn_sgemm_f32s8f32_quantize(p.transB, p.N, p.K, B_float.data(), ldb,
                                 p.quantization_rate, 
...
INSTANTIATE_TEST_SUITE_P(
    SgemmF32S8F32QuantizeTests, SgemmF32S8F32QuantizeParamTest,
    ::testing::Values(
        // N, K, ldb_mul, transB, quantization_rate
        SgemmF32S8F32QuantizeParams{4, 4, 1.0f, false, 1.0f}, // B is 32x16 (KxN)
        SgemmF32S8F32QuantizeParams{16, 32, 1.0f, false, 1.0f}, // B is 32x16 (KxN)
        SgemmF32S8F32QuantizeParams{8, 16, 1.2f, false, 1.0f} // ldb padding
    )
);
...
// Test Fixture for SgemmF32S8F32 PackB tests
class SgemmF32S8F32PackBTest : public ::testing::Test {
protected:
...
TEST(SgemmF32S8F32PackBTest, PackBCorrectness) {
    ...
    for (bool transB : {false, true}) {
        ...
        // Function under test
        xdnn_sgemm_f32s8f32_packb(transB, N, K, B_s8.data(), ldb, packed_func.data());
...
void reference_sgemm_f32s8f32_compute(
    ...
    if (alpha != 1.0f) {
        std::cerr << "ERROR: reference_sgemm_f32s8f32_compute only supports alpha == 1.0f. Got alpha=" << alpha << std::endl;
        return;
    }
    if (!(beta == 0.0f || beta == 1.0f)) {
        std::cerr << "ERROR: reference_sgemm_f32s8f32_compute only supports beta == 0.0f or beta == 1.0f. Got beta=" << beta << std::endl;
        return;
    }
...
struct SgemmF32S8F32ComputeParams {
    int M, N, K;
...
class SgemmF32S8F32ComputeTest : public ::testing::Test {
protected:
    std::vector<float> A;
...
class SgemmF32S8F32ComputeParamTest : public SgemmF32S8F32ComputeTest, 
                                     public ::testing::WithParamInterface<SgemmF32S8F32ComputeParams> {};

TEST_P(SgemmF32S8F32ComputeParamTest, XdnnSgemmF32S8F32Compute) {
    SgemmF32S8F32ComputeParams p = GetParam();
    ...
    // Reference
    reference_sgemm_f32s8f32_compute(p.transA, p.M, p.N, p.K, p.alpha, A.data(), lda, 
                                    packedB.data(), scaleB.data(), zeroB.data(), 
                                    p.beta, C_expected.data(), ldc);
    
    // Function under test
    xdnn_sgemm_f32s8f32_compute(p.transA, p.M, p.N, p.K, p.alpha, A.data(), lda, 
                                packedB.data(), scaleB.data(), zeroB.data(), 
...
INSTANTIATE_TEST_SUITE_P(
    SgemmF32S8F32ComputeTests, SgemmF32S8F32ComputeParamTest,
    ::testing::Values(
        SgemmF32S8F32ComputeParams{8, 16, 32, 1.0f, 1.0f, false, 1.0f, 0.0f},  // beta = 0
        SgemmF32S8F32ComputeParams{8, 16, 32, 1.0f, 1.0f, false, 1.0f, 1.0f},  // beta = 1
        SgemmF32S8F32ComputeParams{16, 8, 32, 1.0f, 1.0f, false, 1.0f, 0.0f},  // different dimensions
        SgemmF32S8F32ComputeParams{8, 16, 32, 1.0f, 1.2f, false, 1.0f, 0.0f},  // ldc padding
        SgemmF32S8F32ComputeParams{8, 16, 32, 1.2f, 1.0f, false, 1.0f, 0.0f},  // lda padding
        SgemmF32S8F32ComputeParams{0, 16, 32, 1.0f, 1.0f, false, 1.0f, 0.0f},  // M = 0
        SgemmF32S8F32ComputeParams{8, 0, 32, 1.0f, 1.0f, false, 1.0f, 0.0f}   // N = 0
    )
);
...
class SgemmF32S8F32FusedOpTest : public ::testing::Test {
protected:
    int M = 4, N = 4, K = 4;
...
TEST_F(SgemmF32S8F32FusedOpTest, ComputeSiLU) {
    ...
    reference_sgemm_f32s8f32_compute(false, M, N, K, 1.0f, A.data(), lda, 
                                    packedB.data(), scaleB.data(), zeroB.data(), 
    ...
    xdnn_sgemm_f32s8f32_compute_silu(false, M, N, K, 1.0f, A.data(), lda, 
                                    packedB.data(), scaleB.data(), zeroB.data(), 
...
TEST_F(SgemmF32S8F32FusedOpTest, ComputeGeLU) {
    // Initialize test matrices
    C_actual.assign(M * ldc, 1.23f);
    C_expected = C_actual;
    ...
    // Step 3: For actual implementation, call the fused function
    xdnn_sgemm_f32s8f32_compute_gelu(false, M, N, K, 1.0f, A.data(), lda, 
                                    packedB.data(), scaleB.data(), zeroB.data(), 
                                    0.0f, C_actual.data(), ldc);
...
TEST_F(SgemmF32S8F32FusedOpTest, ComputeBiasAdd) {
    // Initialize test matrices
    C_actual.assign(M * ldc, 1.23f);
    C_expected = C_actual;
    ...
    // Step 3: For actual implementation, call the fused function
    xdnn_sgemm_f32s8f32_compute_biasadd(false, M, N, K, 1.0f, A.data(), lda, 
                                       packedB.data(), scaleB.data(), zeroB.data(), 
...
TEST_F(SgemmF32S8F32FusedOpTest, ComputeBiasAddReLU) {
    // Initialize test matrices
    C_actual.assign(M * ldc, 1.23f);
    C_expected = C_actual;
    ...
    // Step 3: For actual implementation, call the fused function
    xdnn_sgemm_f32s8f32_compute_biasadd_relu(false, M, N, K, 1.0f, A.data(), lda, 
                                           packedB.data(), scaleB.data(), zeroB.data(), 
                                           0.0f, C_actual.data(), ldc, bias.data());
...
TEST_F(SgemmF32S8F32FusedOpTest, ComputeResidential) {
    // Initialize test matrices
    C_actual.assign(M * ldc, 1.23f);
    C_expected = C_actual;
    ...
    // Step 3: For actual implementation, call the fused function
    xdnn_sgemm_f32s8f32_compute_residential(false, M, N, K, 1.0f, A.data(), lda, 
                                          packedB.data(), scaleB.data(), zeroB.data(), 
...
TEST_F(SgemmF32S8F32FusedOpTest, ComputeResExt) {
    // Initialize test matrices
    C_actual.assign(M * ldc, 1.23f);
    C_expected = C_actual;
    ...
    // Step 3: For actual implementation, call the fused function
    xdnn_sgemm_f32s8f32_compute_resext(false, M, N, K, 1.0f, A.data(), lda, 
                                      packedB.data(), scaleB.data(), zeroB.data(), 
...
TEST_F(SgemmF32S8F32FusedOpTest, ComputeResMul) {
    // Initialize test matrices
    C_actual.assign(M * ldc, 1.23f);
    C_expected = C_actual;
    ...
    // Step 3: For actual implementation, call the fused function
    xdnn_sgemm_f32s8f32_compute_resmul(false, M, N, K, 1.0f, A.data(), lda, 
                                      packedB.data(), scaleB.data(), zeroB.data(), 
