...
void init_matrix(std::vector<XDNN_FP16>& matrix, int rows, int cols, int ld, int seed_offset = 0) {
...
// Test Fixture for HgemmF32U4F32 Quantize tests
class HgemmF32U4F32QuantizeTest : public ::testing::Test {
protected:
    ...
    void verify_quantized_matrices(int K, int N_quant) { // N_quant is N for quantizedB (num elements)
        ...
        // The parameters to xdnn_hgemm_f32u4f32_quantize are N and K for the original B matrix.
        ...
        // The actual call to xdnn_hgemm_f32u4f32_quantize uses ldqb.
...
// This needs to carefully match the logic implied by xdnn_hgemm_f32u4f32_quantize's comments and typical practices.
...
struct HgemmF32U4F32QuantizeParams {
...
class HgemmF32U4F32QuantizeParamTest : public HgemmF32U4F32QuantizeTest,
                                       public ::testing::WithParamInterface<HgemmF32U4F32QuantizeParams> {};

TEST_P(HgemmF32U4F32QuantizeParamTest, XdnnHgemmF32U4F32Quantize) {
    HgemmF32U4F32QuantizeParams p = GetParam();
    ...
    // N for xdnn_hgemm_f32u4f32_quantize is "N" from input, K is "K" from input.
    ...
    xdnn_hgemm_f32u4f32_quantize(p.transB, p.N, p.K, B_float.data(), ldb,
                                 p.quantization_rate, 
...
INSTANTIATE_TEST_SUITE_P(
    HgemmF32U4F32QuantizeTests, HgemmF32U4F32QuantizeParamTest,
    ::testing::Values(
        // N, K, ldb_mul, transB, quantization_rate
        HgemmF32U4F32QuantizeParams{16, 32, 1.0f, false, 1.0f}, // B is 32x16 (KxN)
        HgemmF32U4F32QuantizeParams{8, 16, 1.2f, false, 1.0f} // ldb padding
    )
);
...
// Test Fixture for HgemmF32U4F32 PackB tests
class HgemmF32U4F32PackBTest : public ::testing::Test {
protected:
...
TEST(HgemmF32U4F32PackBTest, PackBCorrectness) {
    // Test a few shapes and both transB cases
    const int N = 4, K = 4; // N must be even for xdnn_hgemm_f32u4f32_packb
    for (bool transB : {false, true}) {
        ...
        // Function under test
        xdnn_hgemm_f32u4f32_packb(transB, N, K, B_u4.data(), ldb, packed_func.data());
        // Check
        for (int i = 0; i < K * N; ++i) {
...
void reference_hgemm_f32u4f32_compute(
    ...
    // Enforce constraints
    if (alpha != 1.0f) {
        std::cerr << "ERROR: reference_hgemm_f32u4f32_compute only supports alpha == 1.0f. Got alpha=" << alpha << std::endl;
        return;
    }
    if (!(beta == 0.0f || beta == 1.0f)) {
        std::cerr << "ERROR: reference_hgemm_f32u4f32_compute only supports beta == 0.0f or 1.0f. Got beta=" << beta << std::endl;
        return;
    }
    if ((M % 2 != 0) || (N % 2 != 0) || (K % 2 != 0)) {
        std::cerr << "ERROR: reference_hgemm_f32u4f32_compute only supports even M, N, K. Got M=" << M << ", N=" << N << ", K=" << K << std::endl;
        return;
    }
    if (M == 0 || N == 0) return;
    for (int m = 0; m < M; ++m) {
...
struct HgemmF32U4F32ComputeParams {
    int M, N, K;
...
class HgemmF32U4F32ComputeTest : public ::testing::Test {
protected:
    std::vector<float> A;
...
class HgemmF32U4F32ComputeParamTest : public HgemmF32U4F32ComputeTest, public ::testing::WithParamInterface<HgemmF32U4F32ComputeParams> {};

TEST_P(HgemmF32U4F32ComputeParamTest, XdnnHgemmF32U4F32Compute) {
    HgemmF32U4F32ComputeParams p = GetParam();
    ...
    // Reference
    reference_hgemm_f32u4f32_compute(p.transA, p.M, p.N, p.K, p.alpha, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), p.beta, C_expected.data(), ldc);
    // Function under test
    xdnn_hgemm_f32u4f32_compute(p.transA, p.M, p.N, p.K, p.alpha, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), p.beta, C_actual.data(), ldc);
    // Verify
    verify_C(p.M, p.N, ldc);
}

INSTANTIATE_TEST_SUITE_P(
    HgemmF32U4F32ComputeTests, HgemmF32U4F32ComputeParamTest,
    ::testing::Values(
        HgemmF32U4F32ComputeParams{8, 16, 32, 1.0f, 1.0f, false, 1.0f, 0.0f},
        HgemmF32U4F32ComputeParams{8, 16, 32, 1.0f, 1.0f, false, 1.0f, 1.0f}
    )
);

// Note: The direct interface function xdnn_hgemm_f32u4f32() is not tested here
...
class HgemmF32U4F32FusedOpTest : public ::testing::Test {
protected:
    int M = 4, N = 4, K = 4;
    ...
    void SetUp() override {
        ...
        xdnn_hgemm_f32u4f32_quantize(false, N, K, B_float.data(), N, 1.0f, packedB.data(), (K + 1) / 2, scaleB.data(), zeroB.data());
        // Bias and res
...
TEST_F(HgemmF32U4F32FusedOpTest, ComputeSiLU) {
    C_actual.assign(M * ldc, 1.23f); C_expected = C_actual;
    reference_hgemm_f32u4f32_compute(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_expected.data(), ldc);
    apply_silu_to_matrix(C_expected, M, N, ldc);
    xdnn_hgemm_f32u4f32_compute_silu(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_actual.data(), ldc);
    for (int i = 0; i < M * N; ++i) EXPECT_NEAR(C_actual[i], C_expected[i], FP32_PRECISION_TOLERANCE);
}
TEST_F(HgemmF32U4F32FusedOpTest, ComputeGeLU) {
    C_actual.assign(M * ldc, 1.23f); C_expected = C_actual;
    reference_hgemm_f32u4f32_compute(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_expected.data(), ldc);
    apply_gelu_to_matrix(C_expected, M, N, ldc);
    xdnn_hgemm_f32u4f32_compute_gelu(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_actual.data(), ldc);
    for (int i = 0; i < M * N; ++i) EXPECT_NEAR(C_actual[i], C_expected[i], FP32_PRECISION_TOLERANCE);
}
TEST_F(HgemmF32U4F32FusedOpTest, ComputeBiasAdd) {
    C_actual.assign(M * ldc, 1.23f); C_expected = C_actual;
    reference_hgemm_f32u4f32_compute(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_expected.data(), ldc);
    apply_bias_add_to_matrix(C_expected, M, N, ldc, bias);
    xdnn_hgemm_f32u4f32_compute_biasadd(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_actual.data(), ldc, bias.data());
    for (int i = 0; i < M * N; ++i) EXPECT_NEAR(C_actual[i], C_expected[i], FP32_PRECISION_TOLERANCE);
}
TEST_F(HgemmF32U4F32FusedOpTest, ComputeBiasAddReLU) {
    C_actual.assign(M * ldc, 1.23f); C_expected = C_actual;
    reference_hgemm_f32u4f32_compute(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_expected.data(), ldc);
    apply_bias_add_to_matrix(C_expected, M, N, ldc, bias);
    apply_relu_to_matrix(C_expected, M, N, ldc);
    xdnn_hgemm_f32u4f32_compute_biasadd_relu(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_actual.data(), ldc, bias.data());
    for (int i = 0; i < M * N; ++i) EXPECT_NEAR(C_actual[i], C_expected[i], FP32_PRECISION_TOLERANCE);
}
TEST_F(HgemmF32U4F32FusedOpTest, ComputeResidential) {
    C_actual.assign(M * ldc, 1.23f); C_expected = C_actual;
    reference_hgemm_f32u4f32_compute(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_expected.data(), ldc);
    apply_residential_to_matrix(C_expected, M, N, ldc, bias, res, ldres);
    xdnn_hgemm_f32u4f32_compute_residential(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_actual.data(), ldc, bias.data(), res.data(), ldres);
    for (int i = 0; i < M * N; ++i) EXPECT_NEAR(C_actual[i], C_expected[i], FP32_PRECISION_TOLERANCE);
}
TEST_F(HgemmF32U4F32FusedOpTest, ComputeResExt) {
    C_actual.assign(M * ldc, 1.23f); C_expected = C_actual;
    reference_hgemm_f32u4f32_compute(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_expected.data(), ldc);
    apply_resext_to_matrix(C_expected, M, N, ldc, bias, gamma, res, ldres);
    xdnn_hgemm_f32u4f32_compute_resext(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_actual.data(), ldc, bias.data(), gamma, res.data(), ldres);
    for (int i = 0; i < M * N; ++i) EXPECT_NEAR(C_actual[i], C_expected[i], FP32_PRECISION_TOLERANCE);
}
TEST_F(HgemmF32U4F32FusedOpTest, ComputeResMul) {
    C_actual.assign(M * ldc, 1.23f); C_expected = C_actual;
    reference_hgemm_f32u4f32_compute(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_expected.data(), ldc);
    apply_resmul_to_matrix(C_expected, M, N, ldc, res, ldres);
    xdnn_hgemm_f32u4f32_compute_resmul(false, M, N, K, 1.0f, A.data(), lda, packedB.data(), scaleB.data(), zeroB.data(), 0.0f, C_actual.data(), ldc, res.data(), ldres);
    for (int i = 0; i < M * N; ++i) EXPECT_NEAR(C_actual[i], C_expected[i], FP32_PRECISION_TOLERANCE);
}