// filepath: /home/zhiyis/workspace/code/xdnn-reloaded/tests/test_hgemm_f16f16f32.cpp
...
void reference_packb_f16f16f32(bool transB, int N, int K, const XDNN_FP16* B, int ldb, XDNN_FP16* packedB) {
...
// Reference implementation for xdnn_hgemm_f16f16f32_compute
void xdnn_hgemm_f16f16f32_compute_reference(bool transA, int M, int N, int K,
...
struct HGEMMComputeF16F16F32TestParams {
    bool transA;
    int M, N, K;
    int lda, ldc;
    float alpha, beta;
    ...
    HGEMMComputeF16F16F32TestParams(bool ta, int m, int n, int k, int la, int lc, float a, float b, const std::string& desc)
        : transA(ta), M(m), N(n), K(k), lda(la), ldc(lc), alpha(a), beta(b), description(desc) {}
};

// Parameterized test class for xdnn_hgemm_f16f16f32_compute function
class HGEMMComputeF16F16F32Test : public ::testing::TestWithParam<HGEMMComputeF16F16F32TestParams> {
protected:
    void SetUp() override {
...
TEST_P(HGEMMComputeF16F16F32Test, HGEMMComputeF16F16F32FunctionTest) {
    const HGEMMComputeF16F16F32TestParams& params = GetParam();
    
    // Print test parameters
    std::cout << "\n=== HGEMMComputeF16F16F32FunctionTest: " << params.description << " ===\n";
    ...
    // Allocate matrices
    std::vector<XDNN_FP16> A, B, packedB;
    ...
    xdnn_hgemm_f16f16f32_packb(false, params.N, params.K, B.data(), params.N, packedB.data());
    ...
    xdnn_hgemm_f16f16f32_compute_reference(params.transA, params.M, params.N, params.K,
                                 params.alpha, A.data(), params.lda, packedB.data(),
                                 params.beta, C_expected.data(), params.ldc);
    
    // Run actual implementation
    xdnn_hgemm_f16f16f32_compute(params.transA, params.M, params.N, params.K,
                       params.alpha, A.data(), params.lda, packedB.data(),
...
INSTANTIATE_TEST_SUITE_P(
    HGEMMComputeF16F16F32FunctionTests,
    HGEMMComputeF16F16F32Test,
    ::testing::Values(
        // Required test cases from the user
        HGEMMComputeF16F16F32TestParams(false, 20, 4096, 1024, 1024, 4096, 1.0f, 0.0f, "required_case_M20_N4096_K1024"),
        HGEMMComputeF16F16F32TestParams(false, 20, 6144, 1024, 1024, 6144, 1.0f, 0.0f, "required_case_M20_N6144_K1024"),
        HGEMMComputeF16F16F32TestParams(false, 1, 4096, 1024, 1024, 4096, 1.0f, 0.0f, "required_case_M1_N4096_K1024"),
        HGEMMComputeF16F16F32TestParams(false, 1, 6144, 1024, 1024, 6144, 1.0f, 0.0f, "required_case_M1_N6144_K1024"),
        
        // Additional test cases for comprehensive coverage
        
        // Basic small test cases
        HGEMMComputeF16F16F32TestParams(false, 16, 16, 16, 16, 16, 1.0f, 0.0f, "basic_small_16x16x16_beta0"),
        HGEMMComputeF16F16F32TestParams(false, 32, 32, 32, 32, 32, 1.0f, 1.0f, "basic_small_32x32x32_beta1"),

        // Different matrix shapes
        HGEMMComputeF16F16F32TestParams(false, 128, 256, 64, 64, 256, 1.0f, 0.0f, "rectangular_128x256x64"),
        HGEMMComputeF16F16F32TestParams(false, 256, 128, 64, 64, 128, 1.0f, 0.0f, "rectangular_256x128x64"),
        HGEMMComputeF16F16F32TestParams(false, 64, 128, 256, 256, 128, 1.0f, 0.0f, "rectangular_64x128x256"),
        
        // Edge cases with small dimensions
        HGEMMComputeF16F16F32TestParams(false, 1, 1, 1, 1, 1, 1.0f, 1.0f, "edge_case_1x1x1"),
        HGEMMComputeF16F16F32TestParams(false, 1, 1024, 512, 512, 1024, 1.0f, 1.0f, "edge_case_1x1024x512"),
        HGEMMComputeF16F16F32TestParams(false, 1024, 1, 512, 512, 1, 1.0f, 1.0f, "edge_case_1024x1x512"),
        HGEMMComputeF16F16F32TestParams(false, 512, 512, 1, 1, 512, 1.0f, 1.0f, "edge_case_512x512x1"),
        
        // Stride variations (non-minimal strides)
        HGEMMComputeF16F16F32TestParams(false, 64, 64, 64, 128, 128, 1.0f, 1.0f, "stride_variation_64x64x64"),
        HGEMMComputeF16F16F32TestParams(false, 32, 32, 32, 64, 64, 1.0f, 1.0f, "stride_variation_32x32x32"),
        
        // Medium-sized matrices
        HGEMMComputeF16F16F32TestParams(false, 512, 512, 512, 512, 512, 1.0f, 1.0f, "medium_512x512x512"),
        HGEMMComputeF16F16F32TestParams(false, 256, 1024, 256, 256, 1024, 1.0f, 1.0f, "medium_256x1024x256"),
        HGEMMComputeF16F16F32TestParams(false, 1024, 256, 256, 256, 256, 1.0f, 1.0f, "medium_1024x256x256"),
        
        // Test cases similar to the required ones but with variations
        HGEMMComputeF16F16F32TestParams(false, 10, 4096, 1024, 1024, 4096, 1.0f, 1.0f, "variation_M10_N4096_K1024"),
        HGEMMComputeF16F16F32TestParams(false, 20, 2048, 1024, 1024, 2048, 1.0f, 1.0f, "variation_M20_N2048_K1024"),
        HGEMMComputeF16F16F32TestParams(false, 20, 4096, 512, 512, 4096, 1.0f, 1.0f, "variation_M20_N4096_K512"),
        
        // Additional specific test case requested by user
        HGEMMComputeF16F16F32TestParams(false, 1, 151936, 1024, 1024, 151936, 1.0f, 0.0f, "user_requested_M1_N151936_K1024")
    )
);

// Additional specific test for edge cases and error conditions
class HGEMMComputeF16F16F32EdgeCaseTest : public ::testing::Test {
protected:
    void SetUp() override {
        srand(12345);
    }
...
TEST_F(HGEMMComputeF16F16F32EdgeCaseTest, ZeroBetaTest) {
    ...
    xdnn_hgemm_f16f16f32_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), N);
...
TEST_F(HGEMMComputeF16F16F32EdgeCaseTest, TransANotSupportedTest) {
    ...
    EXPECT_THROW(
        xdnn_hgemm_f16f16f32_compute_reference(true, M, N, K, 1.0f, A.data(), M, packedB.data(), 0.0f, C.data(), N),
        std::runtime_error
    );
}

// Test structure for packb function parameters
struct HGEMMPackBF16F16F32TestParams {
    bool transB;
    int N, K;
    int ldb;
    std::string description;
    
    HGEMMPackBF16F16F32TestParams(bool tb, int n, int k, int lb, const std::string& desc)
        : transB(tb), N(n), K(k), ldb(lb), description(desc) {}
};

// Parameterized test class for xdnn_hgemm_f16f16f32_packb function
class HGEMMPackBF16F16F32Test : public ::testing::TestWithParam<HGEMMPackBF16F16F32TestParams> {
protected:
    void SetUp() override {
        // Initialize random seed for reproducible tests
        srand(12345);
...
TEST_P(HGEMMPackBF16F16F32Test, HGEMMPackBF16F16F32FunctionTest) {
    const HGEMMPackBF16F16F32TestParams& params = GetParam();
    ...
    // Print test parameters
    std::cout << "\n=== HGEMMPackBF16F16F32FunctionTest: " << params.description << " ===\n";
    ...
    // Run reference implementation
    reference_packb_f16f16f32(params.transB, params.N, params.K, B.data(), params.ldb, packedB_expected.data());
    
    // Run actual implementation
    xdnn_hgemm_f16f16f32_packb(params.transB, params.N, params.K, B.data(), params.ldb, packedB_actual.data());
...
INSTANTIATE_TEST_SUITE_P(
    HGEMMPackBF16F16F32FunctionTests,
    HGEMMPackBF16F16F32Test,
    ::testing::Values(
        // Basic small test case for debugging
        HGEMMPackBF16F16F32TestParams(false, 16, 16, 16, "basic_small_N16_K16_notrans"),
        HGEMMPackBF16F16F32TestParams(false, 32, 32, 32, "basic_small_N32_K32_notrans"),
        HGEMMPackBF16F16F32TestParams(false, 64, 64, 64, "basic_medium_N64_K64_notrans"),
        HGEMMPackBF16F16F32TestParams(true, 16, 16, 16, "basic_small_N16_K16_trans"),
        HGEMMPackBF16F16F32TestParams(true, 32, 32, 32, "basic_small_N32_K32_trans"),
        HGEMMPackBF16F16F32TestParams(true, 64, 64, 64, "basic_medium_N64_K64_trans"),
        HGEMMPackBF16F16F32TestParams(false, 256, 64, 256, "rectangular_N256_K64_notrans"),
        HGEMMPackBF16F16F32TestParams(false, 64, 128, 64, "rectangular_N64_K128_notrans"),
        HGEMMPackBF16F16F32TestParams(true, 128, 64, 64, "rectangular_N128_K64_trans"),
        HGEMMPackBF16F16F32TestParams(true, 64, 128, 128, "rectangular_N64_K128_trans"),
        HGEMMPackBF16F16F32TestParams(false, 1, 1, 1, "edge_case_N1_K1_notrans"),
        HGEMMPackBF16F16F32TestParams(true, 1, 1, 1, "edge_case_N1_K1_trans"),
        HGEMMPackBF16F16F32TestParams(false, 64, 64, 128, "stride_variation_N64_K64_notrans"),
        HGEMMPackBF16F16F32TestParams(true, 64, 64, 128, "stride_variation_N64_K64_trans")
    )
);

// Additional specific tests for edge cases and consistency
TEST_F(HGEMMComputeF16F16F32EdgeCaseTest, PackBConsistencyTest) {
    ...
    // Pack both versions
    xdnn_hgemm_f16f16f32_packb(false, N, K, B_original.data(), N, packedB_notrans.data());
    xdnn_hgemm_f16f16f32_packb(true, N, K, B_transposed.data(), K, packedB_trans.data());
...
TEST_F(HGEMMComputeF16F16F32EdgeCaseTest, PackBStrideBehaviorTest) {
    ...
    // Pack both matrices
    xdnn_hgemm_f16f16f32_packb(false, N, K, B_normal.data(), stride_normal, packedB_normal.data());
    xdnn_hgemm_f16f16f32_packb(false, N, K, B_strided.data(), stride_large, packedB_strided.data());
