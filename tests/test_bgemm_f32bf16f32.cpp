...
void fill_fp16_matrix(std::vector<XDNN_BF16>& mat, int rows, int cols, int ld, float start = 1.0f) {
    mat.resize(ld * rows);
    for (int r = 0; r < rows; ++r)
        for (int c = 0; c < cols; ++c)
...
void reference_compute(bool transA, int M, int N, int K, float alpha, const float* A, int lda, const XDNN_BF16* packedB, float beta, float* C, int ldc) {
    for (int m = 0; m < M; ++m) {
        for (int n = 0; n < N; ++n) {
            float sum = 0.0f;
            for (int k = 0; k < K; ++k) {
...
TEST_P(bgemmF32BF16F32PackBTest, PackBCorrectness) {
    ...
    xdnn_bgemm_f32bf16f32_packb(p.transB, p.N, p.K, B.data(), p.ldb, packedB.data(), p.K, p.N);
...
class bgemmF32BF16F32ComputeTest : public ::testing::TestWithParam<ComputeParams> {};
TEST_P(bgemmF32BF16F32ComputeTest, ComputeCorrectness) {
    auto p = GetParam();
    ...
    xdnn_bgemm_f32bf16f32_compute(p.transA, p.M, p.N, p.K, p.alpha, A.data(), p.K, packedB.data(), p.beta, C.data(), p.N);
    for (int i = 0; i < p.M * p.N; ++i) EXPECT_NEAR(C[i], C_ref[i], 1e-3);
}
INSTANTIATE_TEST_SUITE_P(Compute, bgemmF32BF16F32ComputeTest, ::testing::Values(
...
class bgemmF32BF16F32FusedTest : public ::testing::Test {
protected:
    int M = 16, N = 16, K = 16;
...
TEST_F(bgemmF32BF16F32FusedTest, ComputeSiLU) {
    ...
    xdnn_bgemm_f32bf16f32_compute_silu(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(bgemmF32BF16F32FusedTest, ComputeGELU) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] = gelu(C_ref[m * ldc + n]);
    xdnn_bgemm_f32bf16f32_compute_gelu(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(bgemmF32BF16F32FusedTest, ComputeBiasAdd) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] += bias[n];
    xdnn_bgemm_f32bf16f32_compute_biasadd(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data());
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(bgemmF32BF16F32FusedTest, ComputeBiasAddReLU) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] = relu(C_ref[m * ldc + n] + bias[n]);
    xdnn_bgemm_f32bf16f32_compute_biasadd_relu(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data());
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(bgemmF32BF16F32FusedTest, ComputeResidential) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] += bias[n] + res[m * ldc + n];
    xdnn_bgemm_f32bf16f32_compute_residential(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data(), res.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(bgemmF32BF16F32FusedTest, ComputeResExt) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] += bias[n] + gamma * res[m * ldc + n];
    xdnn_bgemm_f32bf16f32_compute_resext(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data(), gamma, res.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(bgemmF32BF16F32FusedTest, ComputeResMul) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] *= res[m * ldc + n];
    xdnn_bgemm_f32bf16f32_compute_resmul(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, res.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
// Small bgemm test
TEST(bgemm_F32BF16F32, Smallbgemm) {
    int M = 1, N = 1, K = 1;
    float a = 2.0f;
    ...
    small_bgemm_f32bf16f32(M, N, K, &a, K, &b, N, &c, N);
