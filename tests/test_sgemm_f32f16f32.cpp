...
void reference_compute(bool transA, int M, int N, int K, float alpha, const float* A, int lda, const XDNN_FP16* packedB, float beta, float* C, int ldc) {
    for (int m = 0; m < M; ++m) {
        for (int n = 0; n < N; ++n) {
            float sum = 0.0f;
            for (int k = 0; k < K; ++k) {
...
class SgemmF32F16F32PackBTest : public ::testing::TestWithParam<PackBParams> {};
TEST_P(SgemmF32F16F32PackBTest, PackBCorrectness) {
    auto p = GetParam();
    ...
    xdnn_sgemm_f32f16f32_packb(p.transB, p.N, p.K, B.data(), p.ldb, packedB.data());
    for (int i = 0; i < p.K * p.N; ++i) EXPECT_EQ(packedB[i], packedB_ref[i]);
}
INSTANTIATE_TEST_SUITE_P(PackB, SgemmF32F16F32PackBTest, ::testing::Values(
...
class SgemmF32F16F32ComputeTest : public ::testing::TestWithParam<ComputeParams> {};
TEST_P(SgemmF32F16F32ComputeTest, ComputeCorrectness) {
    auto p = GetParam();
    std::vector<float> A(p.M * p.K), C(p.M * p.N, 1.23f), C_ref(p.M * p.N, 1.23f);
    std::vector<XDNN_FP16> packedB(p.K * p.N);
    fill_matrix(A, p.M, p.K, p.K);
    ...
    xdnn_sgemm_f32f16f32_compute(p.transA, p.M, p.N, p.K, p.alpha, A.data(), p.K, packedB.data(), p.beta, C.data(), p.N);
    for (int i = 0; i < p.M * p.N; ++i) EXPECT_NEAR(C[i], C_ref[i], 1e-3);
}
INSTANTIATE_TEST_SUITE_P(Compute, SgemmF32F16F32ComputeTest, ::testing::Values(
...
class SgemmF32F16F32FusedTest : public ::testing::Test {
protected:
    int M = 16, N = 16, K = 16;
    int ldc = 16; // Use N for now, but can be larger for padding tests
    std::vector<float> A, C, C_ref, bias, res;
    std::vector<XDNN_FP16> packedB;
    float gamma = 1.5f;
    void SetUp() override {
        A.resize(M * K); fill_matrix(A, M, K, K);
...
TEST_F(SgemmF32F16F32FusedTest, ComputeSiLU) {
    ...
    xdnn_sgemm_f32f16f32_compute_silu(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(SgemmF32F16F32FusedTest, ComputeGELU) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] = gelu(C_ref[m * ldc + n]);
    xdnn_sgemm_f32f16f32_compute_gelu(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(SgemmF32F16F32FusedTest, ComputeBiasAdd) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] += bias[n];
    xdnn_sgemm_f32f16f32_compute_biasadd(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data());
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(SgemmF32F16F32FusedTest, ComputeBiasAddReLU) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] = relu(C_ref[m * ldc + n] + bias[n]);
    xdnn_sgemm_f32f16f32_compute_biasadd_relu(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data());
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(SgemmF32F16F32FusedTest, ComputeResidential) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] += bias[n] + res[m * ldc + n];
    xdnn_sgemm_f32f16f32_compute_residential(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data(), res.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(SgemmF32F16F32FusedTest, ComputeResExt) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] += bias[n] + gamma * res[m * ldc + n];
    xdnn_sgemm_f32f16f32_compute_resext(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data(), gamma, res.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(SgemmF32F16F32FusedTest, ComputeResMul) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] *= res[m * ldc + n];
    xdnn_sgemm_f32f16f32_compute_resmul(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, res.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
...
TEST(SGEMM_F32F16F32, SmallSGEMM) {
    ...
    small_sgemm_f32f16f32(M, N, K, &a, K, &b, N, &c, N);
