...
class HgemmF32F16F32ComputeTest : public ::testing::TestWithParam<ComputeParams> {};
TEST_P(HgemmF32F16F32ComputeTest, ComputeCorrectness) {
    auto p = GetParam();
    ...
    xdnn_hgemm_f32f16f32_compute(p.transA, p.M, p.N, p.K, p.alpha, A.data(), p.K, packedB.data(), p.beta, C.data(), p.N);
    for (int i = 0; i < p.M * p.N; ++i) EXPECT_NEAR(C[i], C_ref[i], 1e-3);
}
INSTANTIATE_TEST_SUITE_P(Compute, HgemmF32F16F32ComputeTest, ::testing::Values(
...
class HgemmF32F16F32FusedTest : public ::testing::Test {
protected:
    int M = 16, N = 16, K = 16;
...
TEST_F(HgemmF32F16F32FusedTest, ComputeSiLU) {
    ...
    xdnn_hgemm_f32f16f32_compute_silu(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(HgemmF32F16F32FusedTest, ComputeGELU) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] = gelu(C_ref[m * ldc + n]);
    xdnn_hgemm_f32f16f32_compute_gelu(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(HgemmF32F16F32FusedTest, ComputeBiasAdd) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] += bias[n];
    xdnn_hgemm_f32f16f32_compute_biasadd(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data());
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(HgemmF32F16F32FusedTest, ComputeBiasAddReLU) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] = relu(C_ref[m * ldc + n] + bias[n]);
    xdnn_hgemm_f32f16f32_compute_biasadd_relu(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data());
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(HgemmF32F16F32FusedTest, ComputeResidential) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] += bias[n] + res[m * ldc + n];
    xdnn_hgemm_f32f16f32_compute_residential(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data(), res.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(HgemmF32F16F32FusedTest, ComputeResExt) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] += bias[n] + gamma * res[m * ldc + n];
    xdnn_hgemm_f32f16f32_compute_resext(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, bias.data(), gamma, res.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
TEST_F(HgemmF32F16F32FusedTest, ComputeResMul) {
    reference_compute(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C_ref.data(), ldc);
    for (int m = 0; m < M; ++m) for (int n = 0; n < N; ++n) C_ref[m * ldc + n] *= res[m * ldc + n];
    xdnn_hgemm_f32f16f32_compute_resmul(false, M, N, K, 1.0f, A.data(), K, packedB.data(), 0.0f, C.data(), ldc, res.data(), ldc);
    for (int m = 0; m < M; ++m)
        for (int n = 0; n < N; ++n)
            EXPECT_NEAR(C[m * ldc + n], C_ref[m * ldc + n], 1e-3);
}
