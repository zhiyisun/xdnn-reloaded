...
// BGEMM implementation for BF16 data types
...
void xdnn_bgemm_bf16bf16bf16(bool transA, bool transB, int M, int N, int K,
    ...
    int block_cols = 16;
    int packedB_size = xdnn_bgemm_bf16bf16bf16_packb_size(N, K, block_rows, block_cols);
    XDNN_BF16 *packedB = new XDNN_BF16[packedB_size / sizeof(XDNN_BF16)];
    
    // Pack matrix B for better cache locality
    xdnn_bgemm_bf16bf16bf16_packb(transB, N, K, B, ldb, packedB, block_rows, block_cols);
    
    // Compute the matrix multiplication
    xdnn_bgemm_bf16bf16bf16_compute(transA, M, N, K, alpha, A, lda, packedB, beta, C, ldc);
    
    // Free temporary packed matrix
    delete[] packedB;
}

int xdnn_bgemm_bf16bf16bf16_packb_size(int N, int K, int block_rows, int block_cols) {
...
void xdnn_bgemm_bf16bf16bf16_packb(bool transB, int N, int K, const XDNN_BF16 *B, int ldb, XDNN_BF16 *packedB,
...
void xdnn_bgemm_bf16bf16bf16_compute(bool transA, int M, int N, int K,
    ...
    if (beta == 0.0f) {
        for (int i = 0; i < M; i++) {
            memset(&C[i * ldc], 0, N * sizeof(XDNN_BF16));
        }
    } else if (beta != 1.0f) {
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
                C[i * ldc + j] = _xdnn_to_bf16(beta * _xdnn_to_float(C[i * ldc + j]));
            }
        }
    }
                    ...
                    for (int jj = 0; jj < jb; jj++) {
                        ...
                        if (transA) {
                            ...
                            for (int kk = 0; kk < kb; kk++) {
                                sum += _xdnn_to_float(A[(k + kk) * lda + (i + ii)]) * 
                                       _xdnn_to_float(packedB[(k/KB * N/NB + j/NB) * KB * NB + kk * NB + jj]);
                            }
                        } else {
                            // A is not transposed, row-major access
                            for (int kk = 0; kk < kb; kk++) {
                                sum += _xdnn_to_float(A[(i + ii) * lda + (k + kk)]) * 
                                       _xdnn_to_float(packedB[(k/KB * N/NB + j/NB) * KB * NB + kk * NB + jj]);
                            }
                        }
                        
                        // Update C with accumulator and alpha scaling
                        float c_val = _xdnn_to_float(C[(i + ii) * ldc + (j + jj)]);
                        C[(i + ii) * ldc + (j + jj)] = _xdnn_to_bf16(c_val + alpha * sum);
                    }
                }
            }
        }
...
void xdnn_bgemm_bf16bf16bf16_compute_silu(bool transA, int M, int N, int K,
    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            ...
            C[i * ldc + j] = _xdnn_to_bf16(silu_activate(val));
        }
    }
}
...
void xdnn_bgemm_bf16bf16bf16_compute_gelu(bool transA, int M, int N, int K,
    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            float val = _xdnn_to_float(C[i * ldc + j]);
            C[i * ldc + j] = _xdnn_to_bf16(gelu_activate(val));
        }
    }
}
...
void xdnn_bgemm_bf16bf16bf16_compute_biasadd_relu(bool transA, int M, int N, int K,
    ...
    xdnn_bgemm_bf16bf16bf16_compute_biasadd(transA, M, N, K, alpha, A, lda, packedB, beta, C, ldc, bias);
...
void small_bgemm_bf16bf16bf16(int M, int N, int K, const XDNN_BF16 *A, int lda, const XDNN_BF16 *B, int ldb,
                             XDNN_BF16 *C, int ldc) {
    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            ...
            for (int k = 0; k < K; k++) {
                sum += _xdnn_to_float(A[i * lda + k]) * _xdnn_to_float(B[k * ldb + j]);
            }
            
            C[i * ldc + j] = _xdnn_to_bf16(sum);
        }
    }
}
